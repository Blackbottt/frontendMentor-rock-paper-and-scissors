...
const rulesDialogBoxId = document.getElementById('rulesDialogBox');
const rulesDialogBoxCloseId = document.getElementById('rulesDialogBoxClose');
const rulesDialogBoxCloseSvg = document.getElementById('rulesDialogBoxCloseSvg');
const rulesDialogBoxClosePath = document.getElementById('rulesDialogBoxClosePath');
...
const userComputerId = document.getElementById('userVsComputer');
...
const rulesDialogBoxOff = () => {
    return rulesDialogBoxId.style.display = 'none';
}
...
const option = (e) => {
        triangle.className = 'triangleOff';
        userComputerId.style.display = 'flex';
        pointers.style.display = 'flex';
        game();

        let option = e.target;

        if(option.id == 'rock' || option.id == 'rockSvg' || option.parentNode.id == 'rock' || option.parentNode.id == 'rockSvg'){
            human = 'rock';
            user.className = 'chosenRock';
            ...
            `;
        } else if(option.id == 'paper' || option.id == 'paperSvg' || option.parentNode.id == 'paper' || option.parentNode.id == 'paperSvg') {
            human = 'paper';
            user.className = 'chosenPaper';
            return user.innerHTML = `
            <div class="circles">
            ...
            `;
        } else if(option.id == 'scissors' || option.id == 'scissorsSvg' || option.parentNode.id == 'scissors' || option.parentNode.id == 'scissorsSvg'){
            human = 'scissors';
            user.className = 'chosenScissors';
            return user.innerHTML = `
            <div class="circles">
...
const game = () => {
    ...
    if(computer != user){
        ...
        return playAgain.textContent = 'RERUN';
    }
}

const computersChoice = () => {
    console.log('computerThinkingHere...');
    let choices = {
        1: 'rock',
        2: 'paper',
        3: 'scissors'
    }
    // to randomize from 1 to 3
    let choice = Math.floor((Math.random()*3)+1);
    ans = choices[choice];
    computer.className = ans;
    game();
    if(ans == 'rock'){
        computer.className = 'chosenRock'
        return computer.innerHTML = `
            <div class="circles">
            <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48">
                <path
                fill="#3B4262"
        ...
        computer.className = 'chosenScissors'
        return computer.innerHTML = `
            <div class="circles">
            <svg xmlns="http://www.w3.org/2000/svg" width="51" height="58">
            <path
        ...
        computer.className = 'chosenPaper'
        return computer.innerHTML = `
            <div class="circles">
                <svg xmlns="http://www.w3.org/2000/svg" width="49" height="59">
                    <path
...
rules.addEventListener('click', rulesDialogBoxOn, false); 
rulesDialogBoxCloseId.addEventListener('click', rulesDialogBoxOff, false); 
rulesDialogBoxCloseSvg.addEventListener('click', rulesDialogBoxOff, false); 
rulesDialogBoxClosePath.addEventListener('click', rulesDialogBoxOff, false); 
rock.addEventListener('click', option, false); 
paper.addEventListener('click', option, false); 
scissors.addEventListener('click', option, false); 
rock.addEventListener('click', computersChoice, false); 
paper.addEventListener('click', computersChoice, false); 
scissors.addEventListener('click', computersChoice, false); 
